...
def test_phone_layout_has_touch_navigation_and_compact_game_cards() -> None:
    ...
    assert '.st-key-active_sport [role="radiogroup"]' in APP_SOURCE
    assert "min-height: 38px !important;" in APP_SOURCE
    assert ".grid-injury-columns { grid-template-columns: 1fr;" in APP_SOURCE
...
def test_application_copy_uses_direct_forecast_language() -> None:
    ...
    assert "Frozen independent pregame forecast" in APP_SOURCE
