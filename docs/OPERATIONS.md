...
Paid history is opt-in only. Estimate first with `python market_update.py --historical-at <UTC> --dry-run`. The default guard refuses a request above 20 credits. Use targeted weekly checkpoints, deduplicate stored snapshots, and monitor the quota rather than crawling five-minute history.
