...
Production prediction intervals use the standard deviation of chronological residuals available at the release cutoff. Reported interval coverage, pinball loss, and winner calibration use only residuals from validation weeks earlier than the row being scored. They describe historical model error, not every source of real-world uncertainty. Official nflverse injury reports are frozen with each forecast as matchup context and displayed with source-week freshness; they are explicitly not model inputs. A separate 2022-2025 research ablation tested status counts, prior-snap-weighted availability loss, and position-group loss on 723 chronological OOF games. No group improved both the development window and the separate 2025 holdout for either margin or total, so production injury inputs remain rejected. The historical injury feed is also a weekly/final-report proxy rather than a timestamped sequence of every early-week publication.

A dedicated quarterback-replacement experiment separately estimates a starter-to-backup EPA/dropback gap, scales it by expected team dropbacks and injury severity, and applies it after the core football model. The 723-game chronological OOF margin benchmark improved from 10.367 to 10.239 overall and improved injury-event rows from 11.836 to 10.746, but the separate 2025 holdout worsened slightly from 10.062 to 10.100. Totals also worsened. The layer therefore remains research-only; no QB injury adjustment is applied to production forecasts.
